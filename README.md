# 頭城地方記憶典藏 v1.83

這是可直接部署至 GitHub Pages 的完整靜態網站。v1.83 為正式封版版本，統一全站版本、快取參數、校訂資訊與資料指紋。

## 專案身分

- 創站日期：2026-08-02（依本專案最早 Git 提交紀錄）
- 地方編纂與人工校訂：頭城二三事
- 資料整理、程式開發與文字協作：頭城二三事與 ChatGPT
- 本版：v1.83
- 資料更新：2026-08-11
- 時間軸：492 筆，依年代拆檔，每次分批顯示 50 筆

## v1.83 更新重點

- 統一全站版本、快取參數、頁尾回退文字與更新日期。
- 正式封存 492 筆時間軸、28 則小故事、34 個地方地點及 20 個人物／空間／地方專題。
- 重建年代分檔、地點反向關聯、校訂報告與 SHA-256 資料指紋。
- 同步首頁最近更新與資料總數，完成 GitHub Pages 上傳封版。

## v1.82 重點

- 「頭城小故事」改為後加入者優先顯示，讓最新地方記憶先被看見。
- 故事牆改用九種尺寸與色彩節奏組成的密排版面，並保留手機單欄閱讀。
- 「生活飲食」移除香好佳雞排及志明與春嬌炸物店，圖層由13筆調整為11筆。
- 依各店完整地址重新定位11個生活飲食標記，並重建地點與事件反向關聯。
- 全站地點總數由36筆調整為34筆，重新產生地圖資料及版本快取。

## v1.81 重點

- 重新整理「人物與觀點」為人物、地方職人、空間與組織、地方觀點四組入口。
- 補入林志全、Neil Roe、施燦欣、李士儀等人物與職人紀錄，並與微笑灣、衝浪及既有事件互相連結。
- 建立「地方文獻」起始頁，收錄2017年日本出版《宜蘭＋台北》的完整書目、頭城章索引與外部觀看分析。
- 時間軸新增2017年7月18日《宜蘭＋台北》出版紀錄，並從人物與觀點、關於典藏及首頁建立閱讀入口。

- 「頭城小故事」新增盆栽返鄉、1924年頭圍水患、雪隧前長庚交通專車及新舊開蘭路。
- 1924年水患與原有港口紀錄合併補強，不建立重複事件。
- 時間軸新增1895年草嶺古道伏擊與武營記憶、1936年忠魂碑除幕及2019年老街文化藝術季。
- 「人物與觀點」馬偕專題新增〈番仔巴〉史料閱讀，加入暴力內容提醒、歷史歧視用語說明及來源差異。
- 「地方地圖」新增頭城武營忠魂碑及13家雞排、鹽酥雞、炸物與燒烤生活據點。
- 地圖新增「生活飲食」圖層；預設關閉，避免生活標記遮蔽原有歷史地景。
- 重新產生年代分檔、反向地點關聯、校訂報告及SHA-256資料指紋。

## 使用限制

© 2026 頭城二三事／頭城地方記憶典藏。本站原創文字、事件摘要、資料選擇、分類架構、編排方式、策展內容、介面設計及程式碼，除另有標示外，保留所有權利。未經書面同意，不得以自動化工具大量擷取、重製、鏡像、重新發布，或用於建立其他資料庫、商業服務及人工智慧模型訓練。

合理引用請註明完整來源、事件網址與存取日期。個別史料、政府資料、出版內容及影像之權利仍屬原權利人所有；日期、名稱與單純事實不因收錄而成為本站獨占內容。完整條文見 [`copyright.html`](copyright.html)。

## 部署 v1.83

1. 解壓縮 `TouCheng-v1.83-GitHub.zip`。
2. 將資料夾內所有檔案上傳至 GitHub Pages 儲存庫根目錄，取代舊版同名檔案。
3. 確認 `index.html`、`robots.txt`、`copyright.html` 與 `assets/` 位於同一層。
4. 等候 GitHub Pages 完成發布後，重新整理首頁、時間軸、地方地圖、人物與觀點、資料與校訂頁。

注意：在 `bigmax-source.github.io/TouCheng/` 這類專案子路徑下，儲存庫內的 `robots.txt` 不一定能成為整個主機的根層規則。改用自訂網域後，應把它部署於 `https://自訂網域/robots.txt`，並以 Cloudflare AI Crawl Control／WAF 作第二層限制。

## 資料建置與驗證

```bash
python3 tools/build-history.py
node tools/build-places.mjs
```

建置流程會檢查事件來源 ID 不重複，產生年代分檔、校訂報告、地點關聯索引及：

- `assets/data/data-integrity.json`
- `assets/data/data-integrity.js`

本版標準化資料集 SHA-256：

```text
3297943cc848159c242da9d4d5e58c0038ead3fa17e89da895b7c92aa09c96d4
```

## v1.7 Release 說明

使用者尚未把 v1.7 上傳至 GitHub，因此不能把 `v1.7` 標籤錯掛到 v1.61 或 v1.71 的提交。原始 `TouCheng-v1.7.zip` 應先以其真實內容建立對應提交，再建立 `v1.7` Release 並附上該壓縮檔。

原始 v1.7 壓縮檔 SHA-256：

```text
818bd10f2fac4d39326693d0d751e1598f86b5f8b7cc97400fd9967d25fa30b6
```

Release 文字與核對檔見 `RELEASE-v1.7.md`、`SHA256SUMS-v1.7.txt`。
