(() => {
  ...
  function renderDetail(place){
    ...
    const storyLinks=(place.relatedStories || []).length ? `<section class="place-related-stories"><h3>相關閱讀</h3>${place.relatedStories.map(item=>`<a class="story-reading-card" href="stories.html#${encodeURIComponent(item.id)}"><strong>${esc(item.title)}</strong><em>查看更多 →</em></a>`).join('')}</section>` : '';
    const sources=(place.sourceRecords || []).length ? `<section class="place-supplement-sources"><h3>補充研究來源</h3>${place.sourceContext ? `<p>${esc(place.sourceContext)}</p>` : ''}<ul class="archive-source-records">${place.sourceRecords.map(item=>`<li><a href="${esc(item.url)}" target="_blank" rel="noopener noreferrer">${esc(item.name)} ↗</a><p>${esc([item.author,item.date].filter(Boolean).join('；'))}</p>${item.note ? `<p>${esc(item.note)}</p>` : ''}</li>`).join('')}</ul></section>` : '';
    $('#placeDetail').innerHTML=`<div class="place-detail-head"><span class="source-badge ${slugs[place.category]}">${esc(place.category)}</span><button type="button" class="place-close" aria-label="關閉地點介紹">×</button></div><h2>${esc(place.name)}</h2><p>${esc(place.summary)}</p><dl class="place-facts"><div><dt>位置</dt><dd>${esc(place.address)}</dd></div>${isLiving?phoneFact:`<div><dt>關聯</dt><dd>${place.relatedEvents.length}筆時間軸紀錄</dd></div>`}</dl>${isLiving?'<p class="place-data-note">生活資訊可能變動，出發前請向店家或地圖平台再次確認。</p>':`<h3>相關事件</h3>${eventList}`}${storyLinks}${sources}<div class="place-actions">${!isLiving?`<a class="btn btn-primary" href="history.html?query=${encodeURIComponent(place.historyQuery)}">查看此地全部相關紀錄 →</a>`:''}${place.phone?`<a class="btn btn-primary" href="tel:${esc(place.phone.replace(/[^0-9+]/g,''))}">撥打電話</a>`:''}<a class="btn btn-ghost" href="${navigationUrl(place)}" target="_blank" rel="noopener noreferrer">Google Maps 導航 ↗</a>${readingUrl?`<a class="btn btn-ghost" href="${esc(readingUrl)}">閱讀相關專題 →</a>`:''}<a class="place-source" href="${esc(place.sourceUrl)}" target="_blank" rel="noopener noreferrer">查看地點資料來源 ↗</a></div>`;
  }
