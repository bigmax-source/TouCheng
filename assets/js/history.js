(() => {
  ...
  function filtered(){const s=state(),nq=normalize(s.q);return data.filter(x=>{const consolidated=displayCategories(x);const hay=normalize([x.date,x.year,x.dynasty,x.era,x.ganzhi,x.title,x.event,x.source,x.sourceType,...x.categories,...consolidated].join(' '));return(!nq||hay.includes(nq))&&(!s.d||x.dynasty===s.d)&&(!s.c||consolidated.includes(s.c))&&(!s.s||x.recordType===s.s)}).sort((a,b)=>s.order==='asc'?(a.date||a.year).toString().localeCompare((b.date||b.year).toString()):(b.date||b.year).toString().localeCompare((a.date||a.year).toString()))}
  function badge(x){const map={official:['official','官方／公共機構'],local_history:['manuscript','地方文史（待查證）'],community:['supplement','頭城二三事／生活紀錄']};const b=map[x.recordType]||['supplement','其他來源'];return `<span class="source-badge ${b[0]}">${b[1]}</span>`}
