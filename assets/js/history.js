(() => {
  ...
  function placeLinks(x){const links=(window.TOUCHENG_EVENT_PLACES||{})[x.id]||[];return links.length?`<div class="event-place-links"><strong>在地方地圖查看</strong>${links.map(place=>`<a href="map.html#place-${encodeURIComponent(place.id)}">${escapeHTML(place.name)} →</a>`).join('')}</div>`:''}
  function featureLink(x){return x.featureUrl?`<div class="event-feature-link"><a class="btn btn-primary" href="${escapeHTML(x.featureUrl)}">${escapeHTML(x.featureLabel||'閱讀相關專題')} →</a></div>`:''}
  ...
  const archiveId=x=>`TC-H-${x.id}`;
  function card(x,q){const tags=[x.primaryCategory,...(x.tags||[]).filter(t=>topicCategories.includes(t))].filter(Boolean);return `<article class="history-card ${escapeHTML(x.recordType)}" id="event-${escapeHTML(x.id)}" data-year="${escapeHTML(x.year)}"><div class="event-year"><span>${escapeHTML(x.startDate||x.year)}${x.endDate && x.endDate !== x.startDate ? `～${escapeHTML(x.endDate)}` : ''}</span><small>${escapeHTML(x.dynasty)}</small></div><div class="event-body"><div class="event-heading"><div>${badge(x)}<p class="archive-id">典藏編號 <code>${escapeHTML(archiveId(x))}</code></p><p class="event-era">${escapeHTML(x.era)}${x.ganzhi?'・'+escapeHTML(x.ganzhi):''}</p><h2>${highlight(x.title,q)}</h2></div><button class="permalink" type="button" data-copy-link="event-${escapeHTML(x.id)}" aria-label="複製此事件連結">#</button></div><div class="event-tags">${[...new Set(tags)].map(c=>`<button class="event-tag" type="button" data-pick-category="${escapeHTML(c)}">${escapeHTML(c)}</button>`).join('')}</div>${metadata(x)}<p class="event-text">${highlight(x.event,q)}</p>${x.images?.length?`<div class="then-now-grid">${x.images.map(img=>`<figure><img src="${escapeHTML(img.src)}" alt="${escapeHTML(img.alt||'')}" loading="lazy"><figcaption>${escapeHTML(img.caption||'')}</figcaption></figure>`).join('')}</div>`:''}${x.importance?`<div class="why-note"><strong>為什麼記錄？</strong><span>${escapeHTML(x.importance)}</span></div>`:''}${featureLink(x)}${placeLinks(x)}<details class="source-details"><summary>資料來源與校訂狀態</summary><dl><div><dt>典藏編號</dt><dd><code>${escapeHTML(archiveId(x))}</code></dd></div><div><dt>來源</dt><dd>${sourceList(x)}</dd></div><div><dt>可信度</dt><dd>${escapeHTML(x.review?.confidence||'待查證')}</dd></div><div><dt>最後更新</dt><dd>${escapeHTML(x.review?.lastUpdated||'—')}</dd></div></dl><p>${escapeHTML(x.review?.status||x.reviewStatus||'待查證')}</p></details></div></article>`}
  ...
  async function getJSON(url){const response=await fetch(url, {cache: 'no-cache'});if(!response.ok)throw new Error(`${response.status} ${url}`);return response.json()}
